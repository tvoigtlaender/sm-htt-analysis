...
outdir=$(pwd)/output/ml/${ERA}_${CHANNEL}_${TAG}
if [[ ! -d ${outdir} ]]; then
  mkdir ${outdir} -p
fi 
...
then
  ...
  #   0 --balance-batches 1 --conditional 1 #& #--randomization 1
  # python3 htt-ml/training/keras_training.py ${outdir}/dataset_config.yaml \
  ...
  python3 htt-ml/training/keras_training.py ${outdir}/dataset_config.yaml \
    ${FOLD} --balance-batches 1 --conditional 1 #& #--randomization 1
  # python3 htt-ml/training/keras_training.py ${outdir}/dataset_config.yaml \
  ...
  # wait ${TRAIN_PID}
  # kill -9 ${MONITOR_PID}
  python3 htt-ml/training/keras_training.py ${outdir}/dataset_config.yaml ${FOLD} \
  --balance-batches 1 #&
  # TRAIN_PID=$!
  # echo "This is the training: ${TRAIN_PID}"
  # (while [[ True ]]; do ps -aux | grep " ${TRAIN_PID} " | grep training >> ${outdir}/monitor_logs_training1.txt; sleep 10; done;) &

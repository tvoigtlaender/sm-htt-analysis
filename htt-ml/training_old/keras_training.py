    ...
    if (args.balance_batches):
        ...
        class balancedBatchGenerator(Sequence):
            ...
            def __getitem__(self, idx):
                ...
                # if tot_nperClass > self.nperClass:
                #     return 1
                #     print("Was below limit")
                # else:
                #     return 0
                # print("There are {} samples per class in this batch.".format(tot_nperClass))
                #
        ...
        def multithreaded_from_generator(generator, output_signature, parallel_iterations=1):
            ...
            # Distribute batch generation with Round-robin scheduling
                ...
                return tf.data.Dataset.range(parallel_iterations).interleave(dataset_shard, 
                    cycle_length=parallel_iterations, num_parallel_calls=parallel_iterations) #, deterministic=False)
        ...
        # num_takes = 100000
        # max_iter_count = 100
        # iterations = np.zeros(max_iter_count)
        # for elem in traindata_multi.take(num_takes):
        #     if elem >= max_iter_count:
        #         print("very high number of iterations found ({})".format(elem))
        #         limit_counter += 1
        #     else:
        #         iterations[elem] += 1
        # iterations.astype(int)
        # limit_counter += np.sum(iterations[1:])
        # end = time.time()
        # logger.info("Timer stop")
        # logger.info("Elapsed time: {}".format(end - start))
        ...
        # f.write("List of iterations needed: {} \n".format(iterations[:np.where(iterations>0)[0][-1]+1]))
        # f.close()
        # exit(0)
        # define tf.data.Dataset for training data

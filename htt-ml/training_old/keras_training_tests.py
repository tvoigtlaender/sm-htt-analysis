...
def main(args, config):
    ...
    with distribution_strategy.scope():
        ...
        if config["model"]["name"]=="smhtt_dropout_tanh_custom":
            ...
            model = model_impl(len(variables) + len_eras, len(classes), node_num, layer_num)
        else:
    ...
    if (args.balance_batches):
        ...
        class balancedBatchGenerator(Sequence):
            ...
            def __len__(self):
                return eventsPerClassAndBatch * len(self.classes) * len(self.eras) * config["model"]["epochs"]
        ...
        def multithreaded_from_generator(generator, output_signature, parallel_iterations=n_CPU):
            logger.info("Running generator with {} parallel instances.".format(parallel_iterations))
        ...
        # logger.info("Timer stop")
        # logger.info("Elapsed time: {}".format(end - start))
        # logger.info("Number of limit-breaks: {} of {}".format(limit_counter, num_takes))
        # f = open(os.path.join(config["output_path"],"limit_breaks.dat"), "a")
        # f.write("Number of limit-breaks: {} of {}\n".format(limit_counter, num_takes))
        # f.close()
        # exit(0)
        ...
        len_etc = 1
        IndexArray = [[[np.where((z_train[:, i_era] == 1) & (y_train[:, i_class] == 1))[0] for i_class in range(len(classes))] for i_era in range(len(eras))] for i_etc in range(len_etc)]
        tfIndexArray = tf.ragged.constant(IndexArray, dtype=tf.int32)
        print(tfIndexArray.shape)
        tf_len_classes = tf.constant(len(classes))
        tf_len_eras = tf.constant(len(eras))
        tf_len_etc = tf.constant(len_etc)
      
        # @tf.function
        def get_batch(input_para):
            tftotal_classes = tf_len_classes * tf_len_eras * tf_len_etc
            print(tf_len_classes, tf_len_eras, tf_len_etc)
            print(tftotal_classes)
            ...
            print(tfIndexArray.shape)
            # print(tfIndexArray.get_shape())
            for perEtcIndex_i in range(tfIndexArray.shape[0]):
            # for perEraIndex_i in range(tfIndexArray.shape[0]):
                print(tfIndexArray[perEtcIndex_i].shape)
                # print(tfIndexArray[perEraIndex_i].get_shape())
                for perEraIndex_i in range(tfIndexArray[perEtcIndex_i].shape[0]):
                    print(tfIndexArray[perEtcIndex_i][perEraIndex_i].shape)
                    # print(tfIndexArray[perEtcIndex_i][perEraIndex_i].get_shape())
                    for perClassIndex_i in range(tfIndexArray[perEtcIndex_i][perEraIndex_i].shape[0]):
                        print(tfIndexArray[perEtcIndex_i][perEraIndex_i][perClassIndex_i].shape)
                        # print(tfIndexArray[perEtcIndex_i][perEraIndex_i][perClassIndex_i].get_shape())
                        ind_of_class_era = tfIndexArray[perEtcIndex_i][perEraIndex_i][perClassIndex_i]
                        abs_index = (perEtcIndex_i * tf_len_eras + perEraIndex_i) * tf_len_classes + perClassIndex_i
                        ...
                        print(perEtcIndex_i, perEraIndex_i, perClassIndex_i)
                        print(gathered_ind)
        ...
        print(list(get_batch(0)))

        # def filter_batch(input_para):
        #     for class_in_batch_i in range(input_para[2].shape[0]):
        #         batch_weights_mean = tf.math.reduce_mean(input_para[2][class_in_batch_i])
        #         all_batch_weight_mean = all_weights_mean_frac[tf.cast(class_in_batch_i / tf_len_eras, tf.int32)][tf.cast(class_in_batch_i % tf_len_eras, tf.int32)]
        #         if tf.math.less(batch_weights_mean, all_batch_weight_mean):
        #             return False
        #     return True
        ...
        # traindata = traindata.map(get_batch, num_parallel_calls=n_CPU)
        # print(list(traindata.take(1)))
        exit(0)
        # traindata = traindata.filter(filter_batch)
        



        # if len(physical_GPU_devices)>0:
        #     traindata = multithreaded_from_generator(balancedBatchGenerator, output_signature=gen_output_signature, parallel_iterations=n_CPU)

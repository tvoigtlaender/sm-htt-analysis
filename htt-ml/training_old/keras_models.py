...
def smhtt_dropout_tanh_custom(num_inputs, num_outputs, node_num, layer_num): #, seed):
    model = Sequential()
    ...
    for i, nodes in enumerate([node_num] * layer_num):
        if i == 0:
            ...
            model.add(Dense(nodes, kernel_regularizer=l2(1e-5))) #, kernel_initializer=GlorotUniform(seed=seed)
        model.add(Activation("tanh"))
        model.add(Dropout(0.3))

    model.add(Dense(num_outputs, kernel_regularizer=l2(1e-5))) #, kernel_initializer=GlorotUniform(seed=seed)
    model.add(Activation("softmax", dtype="float32"))
    model.compile(loss="categorical_crossentropy",
                  optimizer=Adam(learning_rate=1e-4),
                  weighted_metrics=["mean_squared_error"])
